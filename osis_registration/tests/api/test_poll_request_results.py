...
from osis_registration.tests.factories.user_account_request_result import UserAccountRequestResultFactory
...
class PollRequestResultsTestCase(TestCase):
    ...
    def test_should_retrieve_no_results_when_no_new_results_since_last_poll(self):
        url = reverse(PollRequestResults.name)

        # first call return results
        first_poll_response = self.client.get(url)
        self.assertTrue(first_poll_response.json())

        # second call return empty list
        second_poll_response = self.client.get(url)
        self.assertFalse(second_poll_response.json())
